...
curl 'https://raw.githubusercontent.com/davidmegginson/ourairports-data/main/airport-comments.csv' > $TARGETDIR/AirportComments.csv
...
mysqlimport --user=jerryhobby --password='wjb7ETJ*dzr2jeu2wuf' jerryhobby $TARGETDIR/AirportComments.csv --fields-terminated-by=',' --lines-terminated-by='\n' --columns='id,airport_ref,airport_ident,airport_icao,airport_name,airport_type,airport_description,airport_frequency,airport_runway,airport_latitude,airport_longitude,airport_elevation,airport_continent,airport_iso_country,airport_iso_region,airport_municipality,airport_scheduled_service,airport_gps_code,airport_iata_code,airport_local_code,airport_home_link,airport_wikipedia_link,airport_keywords'
